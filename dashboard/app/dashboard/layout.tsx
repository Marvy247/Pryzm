...
import { Menu, LogOut, Activity, TrendingUp, BarChart3, LayoutDashboard } from 'lucide-react'
...
const navItems = [
  ...
  { href: '/dashboard/event-contracts', name: 'Event Contracts', icon: Activity, badge: 'EC' },
  { href: '/dashboard/positions', name: 'Positions', icon: TrendingUp },
  { href: '/dashboard/track-record', name: 'Track Record', icon: BarChart3 },
]
...
function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const activeItem = navItems.find(n => n.href === pathname) ?? navItems[0]
            ...
            <Tooltip>
              ...
              <TooltipContent side="right" sideOffset={8}>
                <div className="text-left">
                  ...
                  <p className="text-xs text-slate-500">pryzm@agent.local</p>
                </div>
              </TooltipContent>
            </Tooltip>
          ...
          <header className="h-20 flex items-center justify-between px-6 md:px-8 sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-200/50 shadow-sm shadow-slate-900/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Welcome back, <span className="text-sky-600 font-medium">Agent</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 hover:bg-slate-100/50 rounded-xl px-3 py-2 transition-colors group">
                ...
                <span className="text-sm font-medium text-slate-700 hidden md:block group-hover:text-slate-900 transition-colors">Agent</span>
              </button>
            </div>
          </header>
    ...
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center bg-dashboard">
          <div className="animate-pulse text-slate-500">Loading Pryzm...</div>
        </div>
      }
    >
